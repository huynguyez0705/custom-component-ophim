...
jQuery(document).ready(function () {
    ...
    jQuery('#star').raty({
	    ...
	    // Kiểm tra class custom-button và áp dụng max-heigh
});
jQuery(document).ready(function () {
    setTimeout(function () {
        if (jQuery('.custom-button').length) {
            var filmInfo = jQuery('#page-info .text .film-info');
            if (filmInfo.length) {
                filmInfo.css('max-height', '28rem');
            }
        }
    }, 3000); // 3000ms = 3 giây
});

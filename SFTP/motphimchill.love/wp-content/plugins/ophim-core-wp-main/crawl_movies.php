...
function crawl_ophim_page_handle($url)
{
	...
	if (count($sourcePage) > 0) {
		foreach ($sourcePage as $key => $item) {
			array_push($listMovies, get_option('my_custom_api_option','https://ophim1.com') . "/phim/{$item->slug}|{$item->_id}|{$item->modified->time}|{$item->name}|{$item->origin_name}|{$item->year}");
		}
		return join("\n", $listMovies);
	}
	return $listMovies;
...
function crawl_ophim_movies_handle($url, $ophim_id, $ophim_update_time, $filterType, $filterCategory, $filterCountry)
{
	try {
		...
		if ($wp_query->found_posts > 0) { 
			...
			update_post_meta($post->ID, 'ophim_movie_status', getStatus($data['status']));
			update_post_meta($post->ID, 'ophim_fetch_info_url', $data['fetch_url']);
			update_post_meta($post->ID, 'ophim_fetch_ophim_id', $data['fetch_ophim_id']);
			update_post_meta($post->ID, 'ophim_fetch_ophim_update_time', $data['fetch_ophim_update_time']);
// 			update_post_meta($post->ID, 'ophim_original_title', $data['org_title']);
			update_post_meta($post->ID, 'ophim_trailer_url', $data['trailer_url']);
			update_post_meta($post->ID, 'ophim_runtime', $data['duration']);
			update_post_meta($post->ID, 'ophim_episode', $data['episode']);
			update_post_meta($post->ID, 'ophim_total_episode', $data['total_episode']);
			update_post_meta($post->ID, 'ophim_quality', $data['quality']);
			update_post_meta($post->ID, 'ophim_lang', $data['lang']);
			update_post_meta($post->ID, 'ophim_showtime_movies', $data['showtime']);
			$updatepost = array(

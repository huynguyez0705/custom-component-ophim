<?php
/**
 * Plugin Name: Thêm SEO Focus Keyword
...
function run_check_seo_keyword() {
    ...
    $response = "Tổng số bài viết đã kiểm tra: {$results['processed']}\n\n";
    $response .= "Bài viết đã có Focus Keyword:\n";
    if (!empty($results['existing'])) {
        foreach ($results['existing'] as $post) {
            $response .= "Post ID: {$post['post_id']} - Tiêu đề: {$post['post_title']} - Meta Key: {$post['meta_key']} - Giá trị: {$post['meta_value']} - Link chỉnh sửa: {$post['edit_link']}\n";
        }
    } else {
        $response .= "Không có bài viết nào đã có Focus Keyword.\n";
    }

    $response .= "\nBài viết cần cập nhật Focus Keyword (trống):\n";
    if (!empty($results['updated'])) {
        foreach ($results['updated'] as $post) {
            $response .= "Post ID: {$post['post_id']} - Tiêu đề: {$post['post_title']} - Meta Key: {$post['meta_key']} - Giá trị sẽ đặt: {$post['meta_value']} - Link chỉnh sửa: {$post['edit_link']}\n";
        }
    } else {
        $response .= "Không có bài viết nào có giá trị Focus Keyword trống.\n";
    }

    $response .= "\nBài viết cần thêm Focus Keyword mới:\n";
    if (!empty($results['added'])) {
        foreach ($results['added'] as $post) {
            $response .= "Post ID: {$post['post_id']} - Tiêu đề: {$post['post_title']} - Meta Key: {$post['meta_key']} - Giá trị sẽ đặt: {$post['meta_value']} - Link chỉnh sửa: {$post['edit_link']}\n";
        }
    } else {
        $response .= "Không có bài viết nào cần thêm Focus Keyword mới.\n";
    }

    $response .= "\nBài viết bị bỏ qua (không có link chỉnh sửa hợp lệ):\n";
    if (!empty($results['skipped'])) {
        foreach ($results['skipped'] as $post) {
            $response .= "Post ID: {$post['post_id']} - Tiêu đề: {$post['post_title']} - Lý do: {$post['reason']}\n";
        }
    } else {
        $response .= "Không có bài viết nào bị bỏ qua.\n";
    }
...
function run_update_seo_keyword() {
    ...
    $response = "Tổng số bài viết đã xử lý: {$results['processed']}\n\n";
    $response .= "Bài viết đã cập nhật Focus Keyword (trống thành tiêu đề):\n";
    if (!empty($results['updated'])) {
        foreach ($results['updated'] as $post) {
            $response .= "Post ID: {$post['post_id']} - Tiêu đề: {$post['post_title']} - Meta Key: {$post['meta_key']} - Giá trị được đặt: {$post['meta_value']} - Link chỉnh sửa: {$post['edit_link']}\n";
        }
    } else {
        $response .= "Không có bài viết nào có giá trị Focus Keyword trống để cập nhật.\n";
    }
...
function render_seo_keyword_page() {
    ...
    ?>
    <div class="wrap">
        <h1>Thêm SEO Focus Keyword</h1>
        <p>Nhập tiền tố bảng database (mặc định: <?php echo esc_html($default_prefix); ?>) và nhấn nút để kiểm tra hoặc thêm/cập nhật Focus Keyword cho plugin SEO đang active (Rank Math hoặc Yoast SEO).</p>
            ...
            $('#check-keyword-button').click(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        $('#keyword-result').html('<pre>' + response.data + '</pre>');
                    },
                    error: function(response) {
                        $('#keyword-result').html('<p>Lỗi: ' + response.responseJSON.data + '</p>');
                    }
                });
            });

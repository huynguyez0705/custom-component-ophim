...
?>",
    ...
    "keywords": [
        <?php $tags = get_the_terms(get_the_ID(), 'ophim_tags'); if (is_array($tags) && !empty($tags)) { $tag_names = array_unique(array_map(fn($tag) => '"' . esc_html($tag->name) . ' ' . esc_html($domain) . '"', $tags)); echo implode(',', $tag_names); } else { echo '"Đang cập nhật"'; } ?>
    ],
    "aggregateRating": {
